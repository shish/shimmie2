...
class Statistics extends Extension
{
    ...
    public function onPageRequest(PageRequestEvent $event): void
    {
        ...
        if ($event->page_matches("stats") || $event->page_matches("stats/100")) {
            ...
            $upload_tally = [];
            foreach ($this->get_upload_stats() as $name) {
            ...
            if (Extension::is_enabled(CommentListInfo::KEY)) {
                $comment_tally = [];
                foreach ($this->get_comment_stats() as $name) {
            ...
            if (Extension::is_enabled(FavoritesInfo::KEY)) {
                $favorite_tally = [];
                foreach ($this->get_favorite_stats() as $name) {
     ...
     */
    private function get_tag_stats(): array
    {
        global $database;
        // Returns the username and tags from each tag history entry. Excludes Anonymous
        $tag_stats = $database->get_all('SELECT users.name,tag_histories.tags,tag_histories.image_id FROM tag_histories INNER JOIN users ON users.id = tag_histories.user_id WHERE tag_histories.user_id <> 1;');
     ...
     * @return array<string>
     */
    private function get_upload_stats(): array
    {
        global $database;
        // Returns the username of each post, as an array. Excludes Anonymous
        return $database->get_col('SELECT users.name FROM images INNER JOIN users ON users.id = images.owner_id WHERE images.owner_id <> 1;');
    }
    ...
    private function get_comment_stats(): array
    {
        ...
        return $database->get_col('SELECT users.name FROM comments INNER JOIN users ON users.id = comments.owner_id WHERE comments.owner_id <> 1;');
    }

    /**
     * @return array<string>
     */
    private function get_favorite_stats(): array
    {
        global $database;
        // Returns the username of each comment, as an array. Excludes Anonymous
        return $database->get_col('SELECT users.name FROM user_favorites INNER JOIN users ON users.id = user_favorites.user_id WHERE user_favorites.user_id <> 1;');
    }
}
