...
final class CommentList extends Extension
{
    ...
    public function onSearchTermParse(SearchTermParseEvent $event): void
    {
        if ($matches = $event->matches("/^comments(:|<=|<|=|>|>=)(\d+)$/i")) {
            ...
            $event->add_querylet(new Querylet("images.id IN (SELECT image_id FROM comments WHERE owner_id = $user_id)"));
        } elseif ($matches = $event->matches("/^comments_locked[=:](yes|true|on|1)$/i")) {
            $event->add_querylet(new Querylet("images.comments_locked = :true", ["true" => true]));
        } elseif ($matches = $event->matches("/^comments_locked[=:](no|false|off|0)$/i")) {
            $event->add_querylet(new Querylet("images.comments_locked = :false", ["false" => false]));
        }
    }
