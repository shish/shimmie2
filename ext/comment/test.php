...
final class CommentListTest extends ShimmiePHPUnitTestCase
{
    ...
    public function testCommentLockSearch(): void
    {
        self::log_in_as_admin();
        $image_id_1 = $this->post_image("tests/pbx_screenshot.jpg", "locked");
        $image_id_2 = $this->post_image("tests/favicon.png", "unlocked");

        # lock first image
        Ctx::$database->execute("UPDATE images SET comments_locked = TRUE WHERE id = :id", ["id" => $image_id_1]);

        self::assert_search_results(["comments_locked=yes"], [$image_id_1]);
        self::assert_search_results(["comments_locked=no"], [$image_id_2]);
    }
}
