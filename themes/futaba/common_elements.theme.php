...
class FutabaCommonElementsTheme extends CommonElementsTheme
{
     ...
     * @param ?query-array $query
     */
    public function futaba_gen_page_link(string $base_url, ?array $query, int $page, string $name): string
    {
     ...
     * @param ?query-array $query
     */
    public function futaba_build_paginator(int $current_page, int $total_pages, string $base_url, ?array $query): string
    {
        $next = $current_page + 1;
        $prev = $current_page - 1;
